...
const { connectDB, insertDoc, makeRecordOfScanFunc } = require('./mongo.js');
const {
  createCardArray,
  acceptedCard,
  rejectedCard,
...
const createsMongoObjectIds = () => {
  const document = insertDoc({ uid: 'test-card' });
  if (!(document._id instanceof ObjectId)) {
    throw new Error('insertDoc should create a MongoDB ObjectId');
  }
};
